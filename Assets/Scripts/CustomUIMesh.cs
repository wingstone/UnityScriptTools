...
public class CustomUIMesh : MaskableGraphic
{
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        if (uiShape == UISHAPE.Circle)
        {
            ...
            {
                float x = radius * Mathf.Cos(1.0f * i / circleNum * Mathf.PI * 2);
                float y = radius * Mathf.Sin(1.0f * i / circleNum * Mathf.PI * 2);
        ...
        else if (uiShape == UISHAPE.RoundRectangle)
        {
            ...
            {
                float x = -rectTransform.sizeDelta.x * 0.5f + radius - radius * Mathf.Cos(1.0f * i / roundNum * Mathf.PI * 0.5f);
                float y = -rectTransform.sizeDelta.y * 0.5f + radius - radius * Mathf.Sin(1.0f * i / roundNum * Mathf.PI * 0.5f);
                vh.AddVert(new Vector3(x, y), color, Vector2.zero);
            }
